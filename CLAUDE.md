...
- **Reports**: computed fully client-side from IndexedDB (`web/src/lib/report.ts`), so they work offline. `#/reports/print?from=&to=&projects=&print=1` renders the printable report in the device's selected UI language (light sheet via the `<doc-page>` web component in `lib/print/`).
...
- **Task-group edits**: Timer groups share a normalised description, project and tags. The group editor captures member IDs at open, patches only those three fields over the latest rows, and commits every row plus dirty marker in one IndexedDB transaction. Delivery is not server-wide atomic because the sync engine can chunk or isolate rows; exact pushed versions feed the transient N/M result, while the global pending/rejected UI remains authoritative across reloads.
